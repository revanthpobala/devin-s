...
def generate_trade_ideas_for_target(target: Dict[str, Any], position: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    is_open = bool(position and position.get("has_position")) or (target.get("status") == "IN_TRADE") or bool(target.get("is_open_position"))
    ...
    if is_open:
        ...
        stop = float(target.get("tactical_stop") or pos_data.get("stop") or (entry * 0.95))
        t1 = float(target.get("target_1") or pos_data.get("target") or (entry * 1.04))
        t2 = float(target.get("target_2") or (entry * 1.08))
        opt_plan = target.get("options_summary") or target.get("options_structure") or ""

        # 1. Scale-Out & Profit Taking Idea
        ...
        if "LONG" in side:
            ...
            t2_gain = (t2 - entry) * rem_qty if rem_qty > 0 else 0.0
            t2_pct = ((t2 - entry) / entry * 100) if entry > 0 else 0.0
            ...
            if qty >= 2:
                scale_desc = (
                    ...
                    f"to breakeven (${entry:.2f}) to run risk-free into Target 2 (${t2:.2f}, +{t2_pct:.2f}% / +${t2_gain:.2f})."
                )
            else:
                scale_desc = (
        ...
        ideas.append({
            "id": "profit_harvest",
            "category": "PROFIT HARVEST",
            "badge": "SCALE OUT 50%" if qty >= 2 else "TAKE PROFIT",
            "color": "emerald",
            "title": f"Take Profits at Target 1 (${t1:.2f})",
            "action": scale_desc,
            "metrics": f"T1: ${t1:.2f} (+{t1_pct:.2f}%) | T2: ${t2:.2f}",
            "rationale": f"T1: ${t1:.2f} (+{t1_pct:.2f}%) | T2: ${t2:.2f}"
        })

        # 2. Capital Defense / Stop Floor
        ...
        if qty >= 100:
            ...
            opt_action = (
                ...
                f"Sell 1x 30-45 DTE Out-of-the-Money Call (suggested strike ~${t2:.2f}) "
                f"to generate ~1.5%-2.5% annualized cashflow yield while preserving upside to Target 2."
            )
            ...
            opt_action = (
                ...
                f"To monetize or leverage this move without 100 shares: (1) Consider a defined-risk Bull Call Spread ({opt_plan or f'Call Spread targeting ${t1:.2f}'}), "
        ...
        # 4. Retest / Scale-in on Pullback
        ez_low = float(target.get("entry_zone_low") or (entry * 0.99))
        ez_high = float(target.get("entry_zone_high") or entry)
        ideas.append({
            "id": "scale_in",
